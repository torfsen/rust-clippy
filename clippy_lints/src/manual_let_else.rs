...
use clippy_utils::{is_lint_allowed, is_never_expr, peel_blocks, QuestionMarkBlock};
                ...
                IfLetOrMatch::IfLet(if_let_expr, let_pat, if_then, if_else, ..) => {
                    if let Some(ident_map) = expr_simple_identity_map(local.pat, let_pat, if_then)
                        ...
                        && (
                            ...
                            || match QuestionMarkBlock::from_expr(cx, if_else) {
                                Some(block) => !block.can_be_shortened(),
                                None => true,
                            }
                        )
                    {
